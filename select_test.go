...
func TestSelectTemplateRender(t *testing.T) {
	t.Run(
		...
		func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf(
					"Unexpected error preparing templates %v",
					err,
				)
			}
			...
			if result != exp {
				t.Errorf(
					"Expected active item to eq %q, got %q",
					exp,
					result,
				)
			}
			...
			if result != exp {
				t.Errorf(
					"Expected inactive item to eq %q, got %q",
					exp,
					result,
				)
			}
			...
			if result != exp {
				t.Errorf(
					"Expected selected item to eq %q, got %q",
					exp,
					result,
				)
			}
